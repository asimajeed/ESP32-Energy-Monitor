...
def lambda_handler(event, context):
    ...
    for phase in [1, 2, 3]:
        df[f"kWh_phase{phase}"] = voltage * pf * df[f"IRMS{phase}"] * (df["deltaT"] / 3600) / 1000

    df["total_kWh"] = df[[f"kWh_phase{i}" for i in [1, 2, 3]]].sum(axis=1)

    if has_peak:
        df["is_peak"] = df["Time"].apply(lambda x: is_peak_time(x, peak_start_min, peak_end_min))
    ...
    hourly_totals = df.groupby(["date", "hour"])["total_kWh"].sum().reset_index()
    hourly_avg_kWh = hourly_totals.groupby("hour")["total_kWh"].mean().round(4)
    ...
    daily_totals_df["weekday"] = pd.Categorical(daily_totals_df["weekday"], categories=weekday_order, ordered=True)

    weekday_avg_daily_kWh = daily_totals_df.groupby("weekday", observed=True)["daily_kWh"].mean().round(4).sort_index()
    ...
    result = {
        ...
        "hourly_avg_kWh": {"labels": hourly_avg_kWh.index.astype(str).tolist(), "data": hourly_avg_kWh.values.tolist()},
        "daily_kWh": {"labels": [d.strftime("%Y-%m-%d") for d in daily_kWh.index], "data": daily_kWh.values.tolist()},
        "daily_cost_PKR": {"labels": [d.strftime("%Y-%m-%d") for d in daily_cost.index], "data": daily_cost.values.tolist()},
        "weekday_avg_kWh": {"labels": weekday_avg_daily_kWh.index.tolist(), "data": weekday_avg_daily_kWh.values.tolist()},
        "peak_usage_hours": {
